...
<head>
    <meta charset="UTF-8">
    <title>Tạo hợp đồng - Admin</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-group select:focus, .form-group input:focus { border-color: #1565C0; }
...
<body>
    ...
    <div class="main-content">
        <div class="top-bar">
            <div class="page-title">➕ Tạo hợp đồng mới</div>
        ...
        <div class="form-card">
            ...
            <div class="form-body">
                ...
                <div class="info-box">
                    📌 <strong>Lưu ý:</strong>
                    <ul>
                        <li>Chỉ hiển thị sinh viên <strong>chưa có hợp đồng hiệu lực</strong></li>
                        <li>Chỉ hiển thị phòng <strong>còn chỗ trống</strong></li>
                        <li>Ngày kết thúc phải sau ngày bắt đầu</li>
                    </ul>
                </div>

                <form action="${pageContext.request.contextPath}/admin/hopdong/add" method="post">
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select name="mssv" required>
                                ...
                                <c:forEach var="sv" items="${sinhVienList}">
                                    <option value="${sv.mssv}">${sv.mssv} - ${sv.hoTen} (${sv.lop})</option>
                                </c:forEach>
                            </select>
                            <c:if test="${empty sinhVienList}">
                                <div style="color:#e53935; font-size:12px; margin-top:5px;">
                                    ⚠️ Không có sinh viên nào chưa có hợp đồng
                                </div>
                            </c:if>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="idPhong" required>
                                ...
                                <c:forEach var="p" items="${phongList}">
                                    <option value="${p.idPhong}">
                                        ${p.tenPhong} (${p.tenToaNha}) - Còn ${p.sucChua - p.soNguoiHienTai} chỗ
                                    </option>
                                </c:forEach>
                            </select>
                            <c:if test="${empty phongList}">
                                <div style="color:#e53935; font-size:12px; margin-top:5px;">
                                    ⚠️ Không có phòng nào còn trống
                                </div>
                            </c:if>
                        </div>
                    </div>
