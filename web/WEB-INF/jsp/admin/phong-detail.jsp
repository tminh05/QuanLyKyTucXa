...
<head>
    <meta charset="UTF-8">
    <title>Chi tiết Phòng - Admin</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .info-value.highlight { color: #1565C0; font-size: 20px; }
        .info-value.price { color: #e53935; }
        
        .status-empty { background: #e8f5e9; color: #2e7d32; padding: 4px 12px; border-radius: 20px; font-size: 12px; display: inline-block; }
        .status-full { background: #ffebee; color: #c62828; padding: 4px 12px; border-radius: 20px; font-size: 12px; display: inline-block; }
        .status-occupied { background: #E3F2FD; color: #1565C0; padding: 4px 12px; border-radius: 20px; font-size: 12px; display: inline-block; }
        ...
        .thietbi-icon { font-size: 28px; }
        .thietbi-name { font-weight: 600; font-size: 13px; }
        .thietbi-status { font-size: 11px; margin-top: 4px; }
...
<body>
    ...
    <div class="main-content">
        <div class="top-bar">
            <div class="page-title">🏠 Chi tiết phòng</div>
        ...
        <div class="detail-card">
            <div class="card-header">
                ...
                <a href="${pageContext.request.contextPath}/admin/phong" class="btn-back">← Quay lại</a>
            </div>
            <div class="info-grid">
                <div class="info-item">
                    <div class="info-label">Tên phòng</div>
                    <div class="info-value highlight">${phong.tenPhong}</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Tòa nhà</div>
                    <div class="info-value">🏢 ${phong.tenToaNha}</div>
                </div>
                <div class="info-item">
                ...
                <div class="info-item">
                    ...
                    <div class="info-value price"><fmt:formatNumber value="${phong.giaPhong}" type="number" groupingUsed="true"/> đ/tháng</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Sức chứa</div>
                    <div class="info-value">${phong.sucChua} người</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Số người hiện tại</div>
                    <div class="info-value">${phong.soNguoiHienTai} / ${phong.sucChua}</div>
                </div>
            </div>
            <div style="padding: 0 24px 20px 24px;">
                <div class="progress-bar">
                    <div class="progress-fill" style="width: ${phong.soNguoiHienTai * 100 / phong.sucChua}%; 
                        background: ${phong.soNguoiHienTai >= phong.sucChua ? '#e53935' : '#1565C0'};">
                    </div>
                </div>
            </div>
            <div class="info-grid">
            ...
            <div class="action-buttons" style="padding: 20px 24px; border-top: 1px solid #f0f0f0;">
                <a href="${pageContext.request.contextPath}/admin/phong/edit/${phong.idPhong}" class="btn-edit">✏️ Chỉnh sửa</a>
                <a href="${pageContext.request.contextPath}/admin/phong" class="btn-back-list">← Danh sách phòng</a>
            </div>
        </div>
        ...
        <div class="detail-card">
            ...
            <c:choose>
                <c:when test="${empty thietBiList}">
                    <div class="text-center">🔧 Chưa có thông tin thiết bị</div>
                </c:when>
                <c:otherwise>
                        ...
                        <c:forEach var="tb" items="${thietBiList}">
                            <div class="thietbi-item">
                                ...
                                <div>
                                    ...
                                    <div class="thietbi-status ${tb.tinhTrang == 'Bình thường' ? 'status-ok' : 'status-bad'}">
                                        ${tb.tinhTrang}
                                    </div>
                                </div>
                            </div>
        ...
        <div class="detail-card">
            ...
            <c:choose>
                <c:when test="${empty sinhVienList}">
                    <div class="text-center">📭 Không có sinh viên nào trong phòng</div>
                </c:when>
                <c:otherwise>
                    <table>
                        <thead>
                        ...
                        <tbody>
                            <c:forEach var="sv" items="${sinhVienList}">
                                <tr>
                                    ...
                                    <td>${sv.email}</td>
                                    <td>
                                        <a href="${pageContext.request.contextPath}/admin/sinhvien/detail/${sv.mssv}" style="color:#1565C0;">Xem</a>
                                    </td>
                                </tr>
                            </c:forEach>
                        </tbody>
        ...
        <div class="detail-card">
            <div class="card-header">
                <h2>📄 Lịch sử hợp đồng phòng</h2>
            </div>
            <c:choose>
                <c:when test="${empty hopDongList}">
                    <div class="text-center">📭 Chưa có hợp đồng nào cho phòng này</div>
                </c:when>
                <c:otherwise>
                    <table>
                        <thead>
                                    ...
                                    <td>
                                        <c:choose>
                                            <c:when test="${hd.trangThai == 'Hiệu lực'}">
                                                <span class="status-ok">✅ Hiệu lực</span>
                                            </c:when>
                                            <c:otherwise>
                                                <span class="status-bad">⛔ ${hd.trangThai}</span>
                                            </c:otherwise>
                                        </c:choose>
                                    </td>
                                </tr>
